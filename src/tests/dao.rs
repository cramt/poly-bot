...
mod dao {
    use crate::dao::postgres::{
        apply_migrations, ConnectionProvider, DockerConnectionProvider,
    };
    ...
    pub fn discord_id_provider() -> u64 {
        ...
        ATOMIC_DISCORD_ID.load(Ordering::SeqCst)
    }
    ...
    pub async fn ready() {
        ...
        if val.clone() {
            ...
            let mut tables = client.query("SELECT table_name FROM information_schema.tables WHERE table_schema='public'", &[]).await.unwrap().into_iter().map(|x| x.get(0)).collect::<HashSet<String>>();
            while !tables.is_empty() {
                let mut v = vec![];
                for table in &tables {
                    if client.query(format!("DROP TABLE {}", table).as_str(), &[]).await.is_ok() {
                        v.push(table.clone())
                    }
                }
                for x in v {
    ...
    mod user {
        ...
        use crate::tests::dao::dao::{discord_id_provider, ready};
        use std::ops::Deref;
        ...
        async fn add_user() {
            ...
            let _user = users::default()
                .add(UserNoId::new("person", Gender::Femme, None, vec![], discord_id_provider()))
                .await;
        }
